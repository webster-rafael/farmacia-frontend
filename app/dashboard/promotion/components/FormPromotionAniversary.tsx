...
const FormPromotionAniversary = () => {
  ...
  const onSubmitPromotionAniversary = async (
    values: z.infer<typeof formSchema>,
  ) => {
    try {
      const response = await fetch(
        "http://localhost:8000/sendPromotionAniversary",
        {
          method: "POST",
          headers: {
  ...
  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(onSubmitPromotionAniversary)}
        className="w-full space-y-6 rounded-md border bg-white p-4 shadow-md"
      >
        <h1 className="text-2xl font-semibold">Promoção do Aniversariante</h1>
        <FormField
...
export default FormPromotionAniversary;
